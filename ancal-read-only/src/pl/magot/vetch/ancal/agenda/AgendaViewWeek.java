...
import pl.magot.vetch.ancal.database.SimpleCrypto;
			...
			if (row != null)
			{
				...
				for (int iDay = 0; iDay < 7; iDay++)
				{
					...
					if (row.GetVisibleDay(iDay))
					{
						...
						try
						{
							item.AddTimeItem(row.GetStartHour(), row.GetStartMinute(), row.iDurationInMinutes, row.bAllDay, row.bAlarm, SimpleCrypto.decrypt(row.sSubject));
						} catch (Exception e)
						{
							// TODO Auto-generated catch block
							item.AddTimeItem(row.GetStartHour(), row.GetStartMinute(), row.iDurationInMinutes, row.bAllDay, row.bAlarm, row.sSubject);
							e.printStackTrace();
						}
					}
				}					
