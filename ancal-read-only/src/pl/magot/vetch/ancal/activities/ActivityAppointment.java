...
{	
	...
	//private Calendar dateDuration = null;
	...
	private Button btnDurationTime = null;
  ...
  private void InitViews()
  {
  	...
  	edSubject.setOnOpenKeyboard(new TouchEdit.OnOpenKeyboard()
    {
  	...
  	btnDurationTime = (Button)findViewById(R.id.btnAppointmentDurationTime);
  	...
  	btnDurationTime.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				
	    	DurationWidget.Open(ActivityAppointment.this, false, prefs.b24HourMode,
	    			durHour, durMin);
  ...
  private void SetDurationTimeForDayAgendaView()
  {
  	if (getIntent() != null)
  	{
  		Bundle extras = getIntent().getExtras();  	  	
  ...
  private void updateDurationDateTimeForNewAppointment(Calendar calDate)
  {
  	int iHour = calDate.get(Calendar.HOUR_OF_DAY);
  	int iMinute = calDate.get(Calendar.MINUTE);
  ...
  private void InitState()
  {
  	...
  	//dateDuration = Calendar.getInstance();
  	durHour = 1;
  	durMin = 0;
  	dateEndOn = Calendar.getInstance();
  	
  	// REGARDER
  	//dataRow.SetDuration(iMinutesDuration);
      ...
      btnDelete.setVisibility(View.INVISIBLE);
    	...
    	//updateDurationDateTimeForNewAppointment();
    	SetDurationTimeForDayAgendaView();
      ...
      //dateDuration.setTimeInMillis(dataRow.GetDuration()*60000);
      durMin = dataRow.GetDuration() % 60;
      durHour = dataRow.GetDuration() / 60;
  ...
  private void UpdateStartDateTimeView()
  {
  	btnStartDate.setText(utils.GetLongDate(dateStart));
		if (chkAllDay.isChecked())
		{
	  	...
	  	btnDurationTime.setText(utils.GetResStr(R.string.labNoTime));
		} else {
	  	btnStartTime.setText(utils.GetLongTime(dateStart, prefs.b24HourMode));
	  	// PROBLEME
	  	//btnDurationTime.setText(utils.GetLongTime(dateDuration, prefs.b24HourMode));
	  	btnDurationTime.setText(durHour+":"+durMin);
		}	
  }
  ...
  public void SaveData()
  {
  	...
  	//dataRow.SetDuration( dateDuration.getTimeInMillis() / 60000);
  	//Log.d("TOTO","DUREE : "+dateDuration.getTimeInMillis());
  	dataRow.SetDuration(durMin+durHour*60);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data)
  {
	  ...
	  //check for duration widget edit request code
	  	if ((requestCode == DurationWidget.SELECT_TIME_REQUEST) && (resultCode == RESULT_OK))
			{
	  		final int iHour = DurationWidget.GetSelectedTimeHourOnActivityResult(requestCode, resultCode, extras);
	  		final int iMinute = DurationWidget.GetSelectedTimeMinuteOnActivityResult(requestCode, resultCode, extras);    		
		  	//dateDuration.set(Calendar.HOUR_OF_DAY, iHour);
		  	//dateDuration.set(Calendar.MINUTE, iMinute);
		  	durHour = iHour;
		  	durMin = iMinute;
				chkAllDay.setChecked(false);
				UpdateStartDateTimeView();
