...
public class Utils 
{
	...
	public void ShowGetPassword(Activity act)
	{
	  ...
	  //      if (hasFocus) {
	  //          dlg.show().getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_VISIBLE);
	  //      }
	  //  }
	//});
    String out;
    dlg.setView(input);
      ...
      public void onClick(DialogInterface dialog, int which) {
          //EditText input = (EditText) dialog.findViewById(123456789);
      	  EditText input2 = input;
          Editable value = input2.getText();
          pl.magot.vetch.ancal.database.SimpleCrypto.seed = value.toString();   
          activity.init();
		...
		dlg.setIcon(iconId);		
		dlg.create();
		dlg.show().getWindow().setSoftInputMode(LayoutParams.SOFT_INPUT_STATE_VISIBLE);	
	}
